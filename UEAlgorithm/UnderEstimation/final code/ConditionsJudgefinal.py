...
The description of this file:
    #English#
        This file contains a few functions, in which implement the core of the underestimation algorithm.
    In this file, the main part of the algorithm is about dealing with matrix data. In the following
    function contains some parameters which should be understand before you can use it by yourself.
    However, There are some simple explanation about each function's parameters.  
        In order to better understand the common features in every function. I will list some variables
    with explanation.
    
    N: The incoming's usr dimensions
    
    #Chinese#
                    这个文件是低估算法的实现代码，均以函数的形式予以实现。各个函数的主要部分是关于矩阵数据的运算，因此使用python的numpy包中的矩阵相关对象
             函数里面有共性的参数在下边予以说明，其中各个函数形参的简单说明已经在各自的函数文档字符串中给出。
             
    N: 输入的usr的维度，函数中使用到的矩阵的维度，均以此为基准。
    

The change records:
        Time     Changed By    Reason
    2014/5/17        QCQ        To fix the some function's data-deal-segment codes
    2014/5/23        QCQ        To fix the matrix object's diagonal function return a list sometimes,
                                enclosure it(a list) to one matrix object
'''
import rosetta
import numpy as np
import math
...
def ConditionOne(matrix):
    ...
    third_small_vector = np.mat(temporary_matrix[-2, :])
    ...
    compare_vector = (diagonal_vector >= third_small_vector[:,0:-1]) 
...
def ConditionTwo(matrix, support_vector):
    ...
    parameters:
        matrix: the input matrix, which should be a matrix (N + 1) * (N + 2)
        support_vector: the computed support vector 1 * (N + 2)
    ...
    #get the diagonal items to form a vector
    diagonal_vector = np.mat(matrix.diagonal())
    #get the support_vector's last element out.
    compare_vector = (diagonal_vector >= support_vector[0, 0:-1])
...
def IsRegion(matrix, convertedusr_vector):
    ...
    parameters:
        matrix: the input matrix, which should be a matrix (N + 1) * (N + 2)
        convertedusr_vector: The input usr-convert vector 1 * (N + 1)
        
    return:
        boolean: if the element contains in matrix, then return True, otherwise False 
    '''
    flag = True
...
def ConvertUsr(usr, usr_bound):
    ...
    parameters:
        usr: the input usr, which is a list contains multidimensional(N) tuple data of one protein
        usr_bound: this is a list, which items are tuple, in which contains every dimension
                    's lower and upper bound
                    
    return:
        matrix: return a matrix(vector) which is converted usr vector 1 * (N + 1)
    '''
    bound_sum = 0.0
...
def SupportVector(energy, convertedusr_vector, M):
    ...
    parameters:
        ...
        convertedusr_vector: the usr converted vector 1 * (N + 1)
        M: The setted M value, which setted by user 
        
    return:
        matrix: return support vector(matrix) 1* (N + 2)
    '''
    #initial the support vector
    support_vector = np.zeros((1, max(convertedusr_vector.shape) + 1))
    ...
    #store the support_vector's last element with the temp_energy
    support_vector[0, max(convertedusr_vector.shape)] = temp_energy 
