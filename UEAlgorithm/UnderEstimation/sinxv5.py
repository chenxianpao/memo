...
temp_x=[]
temp_y=[]
for index,content in enumerate(np.linspace(0, 13, 100)):
    ...
    for all_tree in range(len(matrix_tree)):
        ...
        if IsRegion(matrix_tree[all_tree], new_vector):
            ...
            for index in range(0,2):
                ...
                lk_vector.append( matrix_tree[all_tree][index,:])
            for xx in range(0,2):
                    #print xx
                temp_x.append(lk_vector[xx][0]*13)
                    #print temp_x[xx]
                    
                    #print sinxx
                temp_y.append(np.sin(temp_x[xx]))
            print temp_y
            print matrix_tree[all_tree]
            print new_vector
            print M
            Estimated_Value = EstimatedValue(temp_y ,matrix_tree[all_tree], new_vector, M)
            del temp_x[:]
            del temp_y[:]
            del lk_vector[:]
