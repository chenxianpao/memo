...
def AcquireRootMatrix(energy_user, usr_bound, M):
    ...
    usr_matrix = np.mat(np.empty((dimension, dimension)))
    root_matrix = np.mat(np.empty((dimension, dimension)))
...
def ConvertUsr(usr, usr_bound):
    ...
    convertedusr_vector[0, len(usr)] = 1 - convertedusr_vector[:,:-1].sum(axis = 1)
