...
def ConditionOne(matrix):
   ...
   # third_small_vector = temporary_matrix[2:3,]
    third_small_vector = temporary_matrix[1:2, 0:-1]
   ...
   # compare_vector = (diagonal_vector >= third_small_vector[0, 0:-1]) 
...
def EstimatedValue(matrix, convertedusr_vector, M):
    ...
    for item in range(max(diagonal_vector.shape)):
        '''
        print "conusr",convertedusr_vector
        print "conusr",convertedusr_vector[0, item]
        print "dia",diagonal_vector
        print "dia",diagonal_vector[item]
        print convertedusr_vector[0, item] / diagonal_vector[item]
        '''
        value_list.append(convertedusr_vector[0, item]*1.0 / diagonal_vector[item])
...
def MatrixReplace(matrix, support_vector):
    ...
    parameters:
        matrix: the input matrix, which should be amatrix (N + 1) * (N + 2)
        support_vector: the computed support vector 1 * (N + 2)
...
def ConvertUsr(usr, usr_bound):
    ...
    parameters:
        usr: the input usr, which is a list contains muti-dimensional(N) tuple data of one protein
        usr_bound: this is a list, which items are tuple, in which contains every dimension
                    's lower and upper bound
