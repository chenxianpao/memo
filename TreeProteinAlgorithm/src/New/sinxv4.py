...
unit_energy1=1+M
unit_energy2=1+M
unit_matrix=np.array([[np.pi*2,0,unit_energy1],[0,np.pi*2,unit_energy2]])

print unit_matrix
child_data = open("child_rmsd_data.txt", "w")
for i,x in enumerate(np.linspace(0, 13, 100)):
...
for index,content in enumerate(np.linspace(0, 2*np.pi, 100)):
...
for i in range(0,100):
    ...
    child_data.write(str(new_x_array[i])+'\t'+str(new_y_array[i])+"\n")
