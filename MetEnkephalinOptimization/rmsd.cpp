...
void RMSD::normalize(double a[3]) {
	double  b;
...
double RMSD::dot(double a[3], double b[3]) {
	return (a[0] * b[0] + a[1] * b[1] + a[2] * b[2]);
}

void RMSD::cross(double a[3], double b[3], double c[3])
{
	a[0] = b[1]*c[2] - b[2]*c[1];
	a[1] = b[2]*c[0] - b[0]*c[2];
	a[2] = b[0]*c[1] - b[1]*c[0];
...
// constructs the correlation R matrix and the E0 value
void RMSD::setup_rotation(double **ref_xlist,
                          double **mov_xlist,
                          int    n_list,
                          double mov_com[3],
                          double mov_to_ref[3],
	...
	/* translate mov_xlist and ref_xlist to centre of mass */
	for (n=0; n<n_list; n++)
		for (i=0; i<3; i++) {
...
/*********************************************************************	
	computes eigenval and eigen_val of a real 3x3 symmetric matrix "a".
	input matrix a (symmetric real matrix RtR): 
						|a11	b		c	|
			matrix(a)=	|b		a22		d	|
						|c		d		a33	|
	...
	return parameters:
			...
			eigenval(d)={¦Ë1	¦Ë2		¦Ë3 }
						...
						|x11	x12		x13	|	
			eigenvac(v)=|x21	x22		x23	| = {X1, X2, X3}
						|x31	x32		x33	|
			n_rot	   = the number of Jacobi rotations that were 
						 needed to achive convergence
	return value:
			0: failure;  1: success
***********************************************************************/
...
int RMSD::jacobi3(double a[3][3], double d[3], double v[3][3], int* n_rot) {
	int count, k, i, j;
...
/****************************************************************	
	...
	input matrix (symmetric real matrix RtR): 
						|a11	b		c	|
			matrix	=	|b		a22		d	|
						|c		d		a33	|
	...
	return paramters:
			...
			eigenval= 	{¦Ë1	¦Ë2		¦Ë3 }, Note: ¦Ë1>¦Ë2>¦Ë3

						|x11	x12		x13	|	
			vec		=	|x21	x22		x23	| = {X1, X2, X3}
						|x31	x32		x33	|
		
						|x11	x21		x31	|	|X1^t|
			engenval=	|x12	x22		x32 | = |X2^t|	
						|x13	x23		x33 |	|X3^t|
	return value:
			0: convergence failed;  1: success
******************************************************************/
int RMSD::diagonalize_symmetric(double matrix[3][3], double eigen_vec[3][3], double eigenval[3]) {
	int n_rot, i, j, k;
	double vec[3][3];
	double val;
	...
	/* descending sort solutions by eigenval*/
	for (i=0; i<3; i++) {
		k = i;
		val = eigenval[i];
...
// calculates the rotation matrix U and the rmsd from the R matrix and E0
int RMSD::calculate_rotation_matrix(double R[3][3],
                                    double U[3][3],
                                    double E0,
                                    double* residual) {
	int i, j, k;
	...
	/* make real symmetric RtR = Rt X R */
	for (i=0; i<3; i++) {
		for (j=0; j<3; j++){
			RtR[i][j] = 0.0;
			for (k = 0; k<3; k++) {
	...
	// right_eigenvec's should be an orthogonal system but could be left or right-handed. Let's force into right-handed system.
	cross(&right_eigenvec[2][0], &right_eigenvec[0][0], &right_eigenvec[1][0]);
	
	// From the Kabsch algorithm, the eigenvec's of RtR are identical to the right_eigenvec's of R. 
...
double RMSD::calculate_rotation_rmsd (double **ref_xlist,
                                      double **mov_xlist,
                                      int n_list,
                                      double mov_com[3],
                                      double mov_to_ref[3],
	...
	setup_rotation(ref_xlist, mov_xlist, n_list, mov_com, mov_to_ref, R, &E0);
	calculate_rotation_matrix(R, U, E0, &residual);
 ...
 *
 */
double RMSD::fast_rmsd(double **ref_xlist,
                       double **mov_xlist,
                       int n_list)
{
	double R[3][3];
    ...
    */
	...
	double rmsd = sqrt( (double) residual*2.0 / ((double) n_list) );


	return rmsd;
}

double RMSD::raw_rmsd(double **ref_xlist, double ** mov_xlist, int n_list) {
	double rmsd = 0;

	for(int i=0; i<n_list; i++) {
		for(int j=0; j<3; j++) {
			rmsd += (mov_xlist[i][j] - ref_xlist[i][j]) * (mov_xlist[i][j] - ref_xlist[i][j]);
		}
	}

	rmsd = rmsd/(double)(n_list);

	return rmsd;
}

double RMSD::raw_translation_rmsd(double **ref_xlist, double **mov_xlist, int n_list) {
	int i, n;
	double rmsd = 0;
	double mov_com[3] = {0};
	double ref_com[3] = {0};
	double mov_to_ref[3] = {0};
	...
	rmsd = raw_rmsd(ref_xlist, mov_xlist, n_list);
